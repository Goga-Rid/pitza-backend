use crate::models::{Order, User, OrderItem};
...
pub async fn get_order_with_items_by_id(req: HttpRequest, pool: web::Data<DbPool>, path: web::Path<i32>) -> impl Responder {
    let mut conn = pool.get().expect("couldn't get db connection from pool");
    let extensions = req.extensions();
    let claims = extensions.get::<Claims>().unwrap();
    let order: Order = orders::table.filter(orders::id.eq(path.into_inner())).filter(orders::user_id.eq(claims.sub)).first(&mut conn).unwrap();
    let order_items: Vec<OrderItem> = order_items::table.filter(order_items::order_id.eq(order.id)).load(&mut conn).unwrap();
...
pub async fn create_order(
    ...
    // Create order items
    let order_items: Vec<NewOrderItem> = order_data.items
        .iter()
        .map(|item| {
            let product_price: BigDecimal = products::table
                .find(item.product_id)
