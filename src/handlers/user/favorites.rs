use crate::models::{NewFavorite, Favorite};
use crate::db::DbPool;
...
pub async fn create_favorite(req: HttpRequest, pool: web::Data<DbPool>, favorite: web::Json<CreateFavorite>) -> impl Responder {
...
pub async fn delete_favorite(req: HttpRequest, pool: web::Data<DbPool>, path: web::Path<i32>) -> impl Responder {
    let mut conn = pool.get().expect("couldn't get db connection from pool");
    let extensions = req.extensions();
    let claims = extensions.get::<Claims>().expect("Claims not found in request extensions");

    let product_ids = path.into_inner();
    println!("Attempting to delete favorite with product_id: {}", product_ids);
    ...
    let deleted = diesel::delete(favorites::table
        .filter(favorites::product_id.eq(product_ids))
        .filter(favorites::user_id.eq(claims.sub)))
        .execute(&mut conn)
        .unwrap();
